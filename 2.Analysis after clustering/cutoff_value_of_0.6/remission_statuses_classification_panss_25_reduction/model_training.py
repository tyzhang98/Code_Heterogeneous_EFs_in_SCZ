...
def optuna_opti(trial, model_type, x, y):
    ...
    if model_type == 'SVM':
        ...
        params['C'] = trial.suggest_float('C', 0.01, 1)
        ...
        params['learning_rate'] = trial.suggest_float('learning_rate', 0.0001, 1)
        clf_ada = AdaBoostClassifier(**params)
        params = {'random_state': 0}
        params['n_estimators'] = trial.suggest_int('n_estimators2', 10, 300)
        params['max_depth'] = trial.suggest_int('max_depth', 1, 60)
    ...
    for _ in tqdm(range(epoch)):
        ...
        for outer_train_idx, outer_test_idx in outer_cv.split(X_train, y_train):
            ...
            for inner_train_idx, inner_test_idx in inner_cv.split(X_train_outer, y_train_outer):
                ...
                study.optimize(lambda trial: optuna_opti(trial, model_name, X_train_inner, y_train_inner), n_trials=50, show_progress_bar=True)
