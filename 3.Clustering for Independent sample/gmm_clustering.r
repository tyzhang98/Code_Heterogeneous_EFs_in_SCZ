...
# 读取数据
data <- read_xlsx("/Users/zhangtongyi/Desktop/1.当前最新版本 /4.聚类分析(重复验证)/0.数据预处理/2.基于对照的 EF z-score 标准化患者的 EF 值，然后基于对照组得到的回归系数校正患者 EF 值的人口学效应 /table/校正并标准化后的患者行为变量数据.xlsx", sheet = 1)
...
# 读取数据
data <- read_xlsx("/Users/zhangtongyi/Desktop/1.当前最新版本 /4.聚类分析(重复验证)/2.GMM/校正并标准化后的患者行为变量数据.xlsx", sheet = 1)
